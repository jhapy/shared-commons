...
public class SphericalUtil {
  ...
  /**
   * Returns the location of origin when provided with a LatLng destination, meters travelled and
   * original heading. Headings are expressed in degrees clockwise from North. This function returns
  ...
  public static LatLng interpolate(LatLng from, LatLng to, double fraction) {
    ...
    if (sinAngle < 1E-6) {
      return new LatLng(
          from.getLat() + fraction * (to.getLat() - from.getLat()),
          from.getLng() + fraction * (to.getLng() - from.getLng()));
    }
  ...
  /**
   * Returns the angle between two LatLngs, in radians. This is the same as the distance on the unit
   * sphere.
   */
   ...
   * determine the orientation of the path. "inside" is the surface that does not contain the South
   * Pole.
   *
   * @param path A closed path.
  ...
  /**
   * Returns the signed area of a closed path on a sphere of given radius. The computed area uses
   * the same units as the radius squared. Used by SphericalUtilTest.
   */
  ...
  /**
   * Returns the signed area of a triangle which has North Pole as a vertex. Formula derived from
   * "Area of a spherical triangle given two edges and the included angle" as per "Spherical
   * Trigonometry" by Todhunter, page 71, section 103, point 2. See http://books.google.com/books?id=3uBHAAAAIAAJ&pg=PA71
