...
public class EarthCalc {
  ...
  public static double harvesineDistance(Point standPoint, Point forePoint) {
    ...
    double a = sin(diffLatitudes / 2) * sin(diffLatitudes / 2) + cos(slat) * cos(flat) * sin(
        diffLongitudes / 2) * sin(diffLongitudes / 2);
  ...
  private static Vincenty vincenty(Point standPoint, Point forePoint) {
    ...
    do {
      ...
      cosλ = cos(λ);
      double sinSqσ =
          (cosU2 * sinλ) * (cosU2 * sinλ) + (cosU1 * sinU2 - sinU1 * cosU2 * cosλ) * (cosU1 * sinU2
              - sinU1 * cosU2 * cosλ);
      sinσ = sqrt(sinSqσ);
      if (sinσ == 0) {
      ...
      λʹ = λ;
      λ = L + (1 - C) * f * sinα * (σ + C * sinσ * (cos2σM + C * cosσ * (-1
          + 2 * cos2σM * cos2σM)));
    ...
    initialBearing =
        (initialBearing + 2 * PI) % (2 * PI); //turning value to trigonometric direction
  ...
  /**
   * Returns the coordinates of a point which is "distance" away from standPoint in the direction of
   * "bearing"
   * <p>
  ...
  private static class Vincenty {
    ...
    /**
     * distance is the distance in meter initialBearing is the initial bearing, or forward azimuth
     * (in reference to North point), in degrees finalBearing is the final bearing (in direction
     * p1→p2), in degrees
     */
